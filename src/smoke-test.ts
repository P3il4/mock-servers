...
const SERVERS: { name: string; port: number; tests: TestCase[] }[] = [
  ...
  { name: 'Replicate', port: 18400, tests: [
    { method: 'POST', path: '/v1/predictions',
      headers: { Authorization: 'Bearer mock' },
      body: { version: 'abc123', input: { prompt: 'a cat' } },
      expectStatus: 201,
      expect: (r: any) => r.id && r.status === 'starting' },
    { method: 'POST', path: '/v1/models/stability-ai/sdxl/predictions',
      headers: { Authorization: 'Bearer mock' },
      body: { input: { prompt: 'a cat' } },
      expectStatus: 201,
      expect: (r: any) => r.id && r.model === 'stability-ai/sdxl' },
    { method: 'GET', path: '/v1/models/stability-ai/sdxl',
      headers: { Authorization: 'Bearer mock' },
      expect: (r: any) => r.owner === 'stability-ai' && r.name === 'sdxl' },
    { method: 'GET', path: '/v1/models',
      headers: { Authorization: 'Bearer mock' },
      expect: (r: any) => Array.isArray(r.results) && r.results.length > 0 },
    { method: 'GET', path: '/_mock/log?last=1',
      expect: (r: any) => Array.isArray(r) },
  ]},
  ...
  { name: 'Replicate (error)', port: 18400, tests: [
    { method: 'POST', path: '/v1/predictions?mock_error=rate_limit',
      headers: { Authorization: 'Bearer mock' },
      body: { version: 'abc', input: { prompt: 'test' } },
      expectStatus: 429,
      expect: (r: any) => r.title === 'Too Many Requests' },
  ]},
